...
async def test_native_value_decodes_seconds_since_midnight(
    ...
    await _poll(
        ...
        freezer,
        mock_neopool_client,
        {**MOCK_POOL_DATA, "filtration1_start": 6 * 3600 + 30 * 60},  # 06:30
    )
...
async def test_native_value_handles_out_of_range_seconds(
    ...
    await _poll(
        ...
        freezer,
        mock_neopool_client,
        {**MOCK_POOL_DATA, "filtration1_start": 86400 + 3600},  # 25:00 -> 01:00
    )
...
async def test_settle_restarts_timer(
    ...
    # 4s elapsed since the first edit, but only 2s since the last: no write yet.
    mock_neopool_client.write_timer.assert_not_awaited()
...
async def test_optimistic_value_survives_overlapping_flush(
    ...
    # Release the first write; it must keep showing the newer optimistic value.
    release.set()
    await first
    state = hass.states.get(entity_id)
    assert state is not None
